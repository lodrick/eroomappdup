...
class BusinessApi {
  ...
  static Future<Token> authenticate(String phoneNumber) async {
    int statusCode;
    Map<String, dynamic> jsonData;
    await http.post(url + 'auth/login', headers: {
      'Accept': 'application/json',
    }, body: {
  ...
  static Future signUp(User user) async {
    int statusCode;
    Map<String, dynamic> jsonData;
    await http.post(url + 'signup', headers: {
      'Accept': 'application/json',
    }, body: {
  ...
  static Future<void> createFavouriteAdvert(userId, advertId, authToken) async {
    int statusCode;

    await http.post(url + '/users/' + userId + '/adverts', headers: {
      'Accept': 'application/json',
      'Authorization': authToken,
    }, body: {}).then((response) {
      statusCode = response.statusCode;
